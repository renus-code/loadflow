      ...
      <div className="d-flex flex-column gap-3">
        ...
        <div className="row g-3">
          <div className="col-md-5">
            <label
              className="small fw-black text-white mb-1 text-uppercase tracking-wider"
              style={{ fontSize: "10px" }}
            >
...
export default function Dashboard() {
  ...
  const availableDrivers = useMemo(() => {
    ...
    const busyDriverIds = new Set(
      loads
        .filter(
          (l) => l.status !== "COMPLETED" && String(l._id) !== editingLoadId,
        )
        .map((l) => {
          const driverId =
  ...
  const onSubmit = async (data: LoadFormData) => {
    try {
      ...
      const payload = {
        ...
        __v: editingLoadId
          ? loads.find((l) => String(l._id) === editingLoadId)?.__v
          : undefined,
      };
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      window.dispatchEvent(
                        new CustomEvent("close-create-load"),
                      );
                    }}
